...
**AWS account structure (as of 2026-07-27).** The app runs in its **own dedicated AWS account, `service-calculator-production` (`847321857537`)**, under `OU-Production` in the OrgFormation-managed organization (baseline + region-control SCPs applied there). It was **migrated out of the shared `acc-test` account (`176971015975`)** — which also hosts an unrelated online shop — because Lambda's concurrent-executions quota is **per-account** (and defaulted to 10, risking cross-app throttling) and for blast-radius isolation, both standard AWS multi-account guidance. The `staging` SST stage currently deploys into this account; the `production` stage (custom domain, production Neon branch) is the next launch step. Full migration runbook + the deploy-role permission fixes it required in `docs/PROGRESS.md` → "AWS account migration".
