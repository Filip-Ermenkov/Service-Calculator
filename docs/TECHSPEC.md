...
>
> **Progress**: Phase 0 (Section 12) is complete and signed off as of 2026-07-04. **Phase 1 is functionally complete as of 2026-07-06**: the mandatory TOTP 2FA slice is committed, and the content model (all collections + globals, EN/FR/DE localization, draft/publish, access control, the LegalInfo publish gate) is built and sandbox-tested, pending Filip's commit + CI run. **One blocker was surfaced by the content-model slice and must be resolved before the next staging deploy: there is no DB-migration step on deploy** (Payload only auto-pushes schema in dev; Lambda runs with push off). See `docs/PROGRESS.md` for the full build/debug history, the migrations-adoption plan, and current handoff state — this document reflects the *plan as corrected by what each phase actually required*; that file explains *how it got there*.
...
- **New, discovered in Phase 1 (content model) — DB migrations must be adopted before the next staging deploy.** Payload's Drizzle adapter only auto-`push`es schema in development; on Lambda (`NODE_ENV=production`) it doesn't, and CI's `deploy-staging` job runs `sst deploy` with no migrate step. Phase 0's Users/Media schema was created by an initial dev-push, so this went unnoticed until the first slice that adds tables. Blocks staging (not local/CI, which push on boot). The adoption plan (add `migrationDir`, generate migrations, `migrate:fresh` the data-less staging branch once to baseline, add a `payload migrate` step before `sst deploy`) is in `docs/PROGRESS.md` → "Immediate next steps". It needs Neon access, so it's owned by Filip and could not be validated from the build sandbox.
