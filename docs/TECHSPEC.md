...
>
> **Progress**: Phase 0 (Section 12) is complete and signed off as of 2026-07-04. **Phase 1 is functionally complete as of 2026-07-06**: the mandatory TOTP 2FA slice is committed, and the content model (all collections + globals, EN/FR/DE localization, draft/publish, access control, the LegalInfo publish gate) is built and sandbox-tested, pending Filip's commit + CI run. **That blocker — no DB-migration step on deploy — is now resolved in code (2026-07-07).** A Payload/Drizzle migrations workflow is adopted: an initial full-schema migration is committed (`src/migrations/`), the CI staging deploy runs `payload migrate` (against Neon's direct/unpooled URL) before `sst deploy`, and the 2FA runtime secrets that were missing from `sst.config.ts` are now wired in. What remains is Filip's one-time staging baseline + first migrated deploy (needs Neon/AWS access — a runbook, not a code gap). See `docs/PROGRESS.md` for the full build/debug history, the migrations workflow, and current handoff state — this document reflects the *plan as corrected by what each phase actually required*; that file explains *how it got there*.
...
> - `Services.calculatorFields`/`formula` are defined as **data only**; the visual Formula/Field Builder over them is Phase 3 (Section 6.4). `Projects.service` is a plain relationship; the §7 "retain the label after the service is deleted" behaviour is a Phase 2 concern (a denormalized snapshot).
...
### 10.5 Database schema migrations (adopted 2026-07-07)
...
| 1 | ✅ **Functionally complete** (2026-07-06), pending commit + a clean CI run — Content model + admin auth | Met: TOTP 2FA committed; `Services`/`Projects`/`CareerListings` collections + `CompanyInfo`/`LegalInfo` globals defined with EN/FR/DE localization, draft/publish, access control, and the LegalInfo publish gate; default Payload admin usable for CRUD; typecheck clean and a Local-API integration suite (`tests/int/content.int.spec.ts`) green. `Translations` intentionally deferred to Phase 5 (see Section 5 status note). **Caveat (resolved 2026-07-07):** reaching *staging* required a DB-migrations workflow — now adopted and sandbox-validated (Section 10.5); the initial migration is committed and CI migrates before deploying. The first staging cutover (one-time baseline + secrets) is a runbook in `docs/PROGRESS.md`. |
