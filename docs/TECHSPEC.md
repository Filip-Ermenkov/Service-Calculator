...
### 6.4 Calculator Field Builder & Formula Builder (admin) — Phase 3 **part 2** (not yet built)
...
> **Status: this is Phase 3 part 2, still to build.** Phase 3 part 1 (§6.3, 2026-07-19) delivered the *evaluator + public calculator* over the *stored* `calculatorFields`/`formula`; part 2 is the non-technical **authoring** UI that produces that data. It targets the **same JSONLogic** `src/lib/pricing/` already evaluates, and its live preview will call the very same `computePrice()` the public page uses (preview == production, by construction — the whole point of the shared pure module). Until it ships, admins edit `calculatorFields` as plain Payload fields and the optional `formula` as raw JSONLogic. Research current best practices for **custom Payload field components** at implementation time (the standing rule).
...
> **✅ Mostly built in Phase 2 part 1 (2026-07-17).** Per-page `generateMetadata` (title/description/canonical/`hreflang` incl. `x-default`/Open Graph), `LocalBusiness` (Home) + `Service` (service pages) JSON-LD, and localized `sitemap.ts`/`robots.ts` are all in (`src/lib/seo.ts`, `src/app/{sitemap,robots}.ts`). **Indexing is an explicit opt-in** (`NEXT_PUBLIC_ALLOW_INDEXING === 'true'`, prod-only) — safer than a hostname check given the dynamic CloudFront staging URL. **Lighthouse CI budget now built (2026-07-18):** `lighthouserc.cjs` + a `Lighthouse CI` step in `ci.yml`'s `verify` job audit a production build across 4 URLs × 3 runs. Accessibility, best-practices, and performance are **hard gates** (calibrated to the first CI baseline: ≥0.9 / ≥0.9 / ≥0.8, all proven 12/12); Core Web Vitals are tracked WARNs. **SEO is a WARN** — its ~0.5–0.58 baseline is *partly* the intentional `noindex` (penalised by Lighthouse; `is-crawlable` turned off) but *also* real audits: `meta-description` gaps on `/legal`/`/privacy`/(empty `/about`) and likely `legible-font-size`/`tap-targets` from the design's small labels. So indexing-at-launch alone won't reach 0.9 — SEO becomes a hard gate only after a **pre-launch SEO + mobile-legibility polish** (tracked in `docs/PROGRESS.md`). **Still a perf item:** switching CMS `<img>` to `next/image` (the `next.config.ts` `localPatterns` groundwork is already in place); tighten the performance floor once that lands.
...
| 3a | Live price calculator + shared evaluator | ✅ **Built + verified on Filip's machine 2026-07-19** (staging deploy + CI axe/Lighthouse on next push). Public calculator computes in **real time** from the stored `formula`, replacing the Phase 2 static preview; pure zero-dependency evaluator `src/lib/pricing/` (standard JSONLogic format, in-house arithmetic interpreter — not the unmaintained `json-logic-js`) shared client/server/PDF; §7 non-positive → "Contact us for a price"; required-field gating; per-locale EUR. `lint`/`typecheck`/`test:int`/`build`/`test:e2e` green + full manual pass. Migration-free. |
| 3b | Calculator field/formula **builder** (admin) | Admin can define fields/formula **visually** in the `Services` editor (§6.4) — a field-list editor + a structured formula builder that serialises to the same JSONLogic `src/lib/pricing/` evaluates, with a live preview calling the same `computePrice()` the public page uses. **Not yet built.** |
