...
>
> **Status as of 2026-07-08**: Phase 0 (infrastructure) and Phase 1 (admin foundation) are complete and **live on the staging site**. Phase 1 delivered **mandatory TOTP 2FA on the admin login** (§5.1) and the **content model** behind every section below — the Services/Projects/Careers collections and the Company Info / Legal Notice + Privacy globals, editable through the admin panel in English with French/German localization ready (§2.1, §5.3–§5.7). What is **not** built yet: the public-facing pages themselves (Home, Projects, Service calculator, About, Careers — **Phase 2, the current focus**), the live price calculator and its visual builder (Phase 3), PDF quotes (Phase 4), the automatic FR/DE translation pipeline and the Translation Management screen (Phase 5), and the contact form (Phase 6). So an administrator can already log in securely (on staging) and manage content; visitors don't yet see it. One safeguard from this document is already enforced in code: the **Legal Notice cannot be published with placeholder details** (§2.5) — the required registration fields block it until they're real. See `docs/PROGRESS.md` for the full build history and current handoff state, and `docs/TECHSPEC.md` §12 for the phase roadmap.
