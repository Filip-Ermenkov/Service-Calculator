...
>
> **Status as of 2026-07-17**: Phase 0 (infrastructure), Phase 1 (admin foundation), and **Phase 2 part 1 (the public site)** are complete and **live on staging**. The public pages described below now exist and render real content managed in the admin: the **Home page** with its service grid (§3.1), each **Service page** (§3.3 — description, disclaimer, and the calculator inputs shown as a *preview*; the live real-time pricing and its visual builder are **Phase 3**), the **Projects**, **About Us**, and **Careers** pages (§3.2, §3.4, §3.5), and the **Legal Notice + Privacy Policy** pages (§2.5). **Language switching is live** with the language in the URL (`/en`, `/fr`, `/de`, §2.1) — the interface is fully translated now; page *content* is shown in English until the automatic FR/DE translation lands in **Phase 5**. The **Projects page now has its live search + service-category filter** (§3.2), results updating instantly as you type or filter, and each project keeps its category label even if that service is later removed (§7). Still to come: the live price calculator (Phase 3), PDF quotes (Phase 4), the FR/DE translation pipeline + Translation Management screen (Phase 5), and the contact form on the About page (Phase 6). One smaller follow-up remains from Phase 2: cookie-free visit **analytics** (§2.5). One safeguard is already enforced end-to-end: the **Legal Notice and Privacy Policy stay hidden ("not yet available") until the real registration details are entered and published** (§2.5) — placeholder legal details can never appear on the live site. See `docs/PROGRESS.md` for the full build history and current handoff state, and `docs/TECHSPEC.md` §12 for the phase roadmap.
...
- That the site uses anonymous, cookie-free visit analytics to understand traffic — no personal profiles, no cross-site tracking, no advertising.
...
**No cookie-consent banner** is shown to visitors. The only cookie in use remembers the visitor's chosen language (strictly functional), and the visit analytics are cookie-free by design — neither requires consent under GDPR/ePrivacy rules, and both facts are disclosed in the Privacy Policy regardless.
