...
> 3. **A security update was applied** (2026-09-10) to the underlying frameworks, closing two critical vulnerabilities published against them, plus one tightening of the admin login so that the protection against password-guessing cannot be cleared without passing the two-factor step.
>
> **What still stands between the site and a public launch is unchanged, and none of it is development work:** the company's real registered legal details must be entered before the Legal Notice and Privacy Policy can be published (a safeguard prevents publishing placeholders); the email sending mailbox must be verified so the contact form and the emailed quote actually deliver — **until that is done the contact form cannot send at all**: a visitor who submits it sees an error asking them to phone or email instead, so nobody's message is quietly lost, but nobody's message gets through either; the anti-spam keys must be switched on; and the site must be made findable by search engines. The site is live at `https://bulbau.lu` but deliberately not yet announced or indexed.
>
> ---
>
> **Previous status (2026-08-06), kept for context**: Phase 0 (infrastructure), Phase 1 (admin foundation), and **all of Phase 2 (the public site — both parts, incl. clean service slugs, the Projects search/category filter, the §7 service-label snapshot, and CI accessibility/performance gates)** are complete. **⭐ The site now runs on its real address, `https://bulbau.lu` (production went live 2026-07-31), in addition to the internal staging copy.** It is **not yet publicly launched/announced** — it is deliberately kept out of Google and other search engines until the final pre-launch checks are done (the company's real legal details, a last accessibility/SEO audit, then the "make it findable" switch). *(Work since 2026-07-21 has been infrastructure/operational, invisible to visitors: the app was moved to its own dedicated AWS hosting account, security hardening landed, and the production site + custom domain were stood up — see `docs/PROGRESS.md`. Since then the two remaining **user-facing** additions have both landed in code: the **FR/DE automatic translations (Phase 5 part 1, live on staging 2026-08-02)** and the **email-the-quote option (Phase 4 part 2, built 2026-08-05 — not yet deployed/verified)**.)* The public pages described below now exist and render real content managed in the admin: the **Home page** with its service grid (§3.1), each **Service page** (§3.3 — description, disclaimer, and a **working real-time price calculator** as of **Phase 3 part 1**, 2026-07-19: fill in the parameters and the estimated total updates live, formatted in euros, with a "Contact us for a price" fallback for edge cases; the *visual admin builder* over those fields is **done as of Phase 3 part 2**, 2026-07-20 — see §5.3), the **Projects**, **About Us**, and **Careers** pages (§3.2, §3.4, §3.5), and the **Legal Notice + Privacy Policy** pages (§2.5). **Language switching is live** with the language in the URL (`/en`, `/fr`, `/de`, §2.1) — the interface is fully translated, and page *content* now **auto-translates to FR/DE on save** (Phase 5 part 1, 2026-08-02) instead of falling back to English, with a dedicated **Translation Management admin screen** to review and correct those translations now built and live in production (Phase 5 part 2, 2026-08-06 — see §5.7). The **Projects page now has its live search + service-category filter** (§3.2), results updating instantly as you type or filter, and each project keeps its category label even if that service is later removed (§7). Also complete: the **visual admin calculator/formula builder** (Phase 3 part 2, 2026-07-20 — the admin now builds a service's pricing formula point-and-click, with a live preview matching the public page). **PDF quotes — the Download path — is built as of Phase 4 part 1 (2026-07-20):** each service page now has a **"Download PDF quote"** button that produces a branded, trilingual PDF of the estimate on demand (never stored), matching what's on screen (§4); the *email-it-to-me* option is **built as of Phase 4 part 2 (2026-08-05)** — an "Email me the quote" button that sends the same PDF to an address the visitor enters — though it is **not yet switched on live**: it still needs the SES sending identity verified, the `EmailSender` set, a deploy, and AWS "production access" (until then AWS only delivers to pre-verified addresses). The website sends these from **`info@bulbau.lu`**; a separate **`office@bulbau.lu`** mailbox (to be set up at an email provider) is where clients reach the owner and where replies go. The real PDF-on-server rendering already runs on staging. The **contact form is now built and live in code (Phase 6, 2026-08-06)** as a dedicated **Contact page** (a deliberate improvement over the original "on the About page" plan — clearer navigation and better for search visibility): visitors can send a message that goes straight to the company inbox, protected invisibly from spam by a Cloudflare "are you human?" check plus a hidden honeypot; it's deployed to production but needs its spam-check keys + a verified sending mailbox switched on to be fully usable (see `docs/PROGRESS.md`). The **Translation Management** admin screen (Phase 5 part 2) is now **built and live in production (2026-08-06)** — a private `/admin/translations` page where the admin reviews every English/French/German string side by side and corrects any translation (typed inline for short text, or opened in the normal editor for longer rich text), so §5.7 is now delivered. **⭐ With that, every user-facing feature in this document is built.** The only things left before the site can be publicly announced are operational switches, not new features: switching on the built **Send-to-Email** option (needs AWS "production access" + the `office@` mailbox), switching on the contact form's live spam-check keys + a verified sending mailbox, entering the company's real legal details, and a final launch/security pass. Web analytics was considered and **deliberately left out** (owner-facing insight only, not required — see `docs/TECHSPEC.md` §6.10); the site stays cookieless with no consent banner. One safeguard is already enforced end-to-end: the **Legal Notice and Privacy Policy stay hidden ("not yet available") until the real registration details are entered and published** (§2.5) — placeholder legal details can never appear on the live site. See `docs/PROGRESS.md` for the full build history and current handoff state, and `docs/TECHSPEC.md` §12 for the phase roadmap.
...
> **✅ Built + deployed to production (Phase 6, 2026-08-06) — as a DEDICATED "Contact" page, not on the About page.** This is a deliberate, recorded deviation: a standalone `/contact` page (linked from the main navigation, the footer, and the Home "Get in Touch"/"Contact Us" buttons) is industry-standard, clearer to find, and better for search visibility than a form buried inside About. The About page keeps its contact-details panel and now points visitors to the Contact page. Fields as below (phone is optional; subject is optional in the build rather than required). The message is relayed straight to the company's inbox and **never stored on the website** (privacy by design). Spam protection is implemented as described in the last bullet. It is deployed but needs its Cloudflare Turnstile keys + a verified sending mailbox switched on per stage to be fully functional (see `docs/PROGRESS.md` → "Phase 6"). The publish gate on the Legal/Privacy pages (§2.5) still applies before this is signed off for public launch.
...
> **✅ Download built in Phase 4 part 1 (2026-07-20); Send-to-Email is part 2.** Each service page with a calculator shows a **"Download PDF quote"** button that generates the PDF on demand and downloads it, entirely on the server and never stored. The document contains everything below — header (company wordmark + phone/email), the prominent estimate-only disclaimer, the service title + date, a table of the parameters the visitor entered with each one's contribution, the prominent total, and a footer repeating the contacts + an invitation to get in touch — and is produced **in the language the visitor had selected** (EN/FR/DE). The price and the §7 "Contact us for a price" edge case are computed by the **same engine as the on-screen calculator**, so the PDF can never disagree with what the visitor saw. The button is available whether or not every field is filled (§3.3); an incomplete estimate simply shows the "contact us" state. **Send-to-Email** (the second delivery option below) is **Phase 4 part 2** — it needs the company's real email domain configured for sending first, so it's deferred until that exists. *(The real server-side PDF rendering runs on a separate, isolated component, now live on staging after its deploy-role permission was applied; locally the same document is shown as a print-ready web preview.)*
...
> **Status (2026-08-06): auto-translation is LIVE (part 1) AND the dedicated Translation Management screen is now BUILT + live in production (part 2, commit `e3121af`).** Phase 5 **part 1** implemented the automatic EN→FR/DE translation-on-save (via AWS Translate — see TECHSPEC §6.7 / PROGRESS.md), so §6's "content is automatically translated and immediately live in all three languages" now works. Phase 5 **part 2** added the dedicated screen described below at `/admin/translations` (linked in the admin nav): one searchable cross-collection place listing every translatable string with its English source beside the current French and German, each marked "Translated" or "Needs translation". Short text is **corrected inline** (typing a value saves it as a manual override that takes effect immediately; a "Re-translate" button regenerates the machine translation from the English); longer rich-text content opens in the normal document editor at the chosen language via an "Edit in editor" link. **One refinement from the description below is intentionally deferred to a later slice:** the screen shows each translation but does not yet *distinguish* a machine translation from a manual override, nor show an automatic "needs review" flag when the English source changes under an override (that needs an extra behind-the-scenes record per string). Everything else in this section — the searchable overview and editing/correcting a translation — is delivered. (Manual overrides also still work the original way, by switching the admin panel's locale and editing a field directly.)
